    ...
    <div className="min-h-screen gradient-bg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <p className="text-sm sm:text-base text-gray-600">Top performers and goal achievers in the community</p>
        </div>
          ...
          {leaderboard.slice(0, 3).map((user, index) => (
            ...
            >
              ...
              <div className="pt-3 sm:pt-4">
                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-avalanche-100 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
                  <span className="text-avalanche-600 font-bold text-lg sm:text-xl">{user.avatar}</span>
                </div>
                <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-1 truncate px-2">{user.name}</h3>
                <p className="text-xs sm:text-sm text-gray-600 mb-3 sm:mb-4 truncate px-2">{user.address}</p>
              ...
              >
                <div className="flex items-center space-x-3 mb-3">
                  <div className="flex items-center space-x-2 flex-shrink-0">
                    {getRankIcon(user.rank)}
                    <span className="font-medium text-gray-900 text-sm">#{user.rank}</span>
                  </div>
                  <div className="w-10 h-10 bg-avalanche-100 rounded-full flex items-center justify-center flex-shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-600 truncate">{user.address}</div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="flex items-center space-x-1">
                    <Trophy className="w-3 h-3 text-warning-600" />
                    <span className="text-gray-600">Rewards:</span>
                    <span className="font-medium text-gray-900">{user.totalRewards}</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <Target className="w-3 h-3 text-success-600" />
                    <span className="text-gray-600">Goals:</span>
                    <span className="font-medium text-gray-900">{user.completedGoals}</span>
                  </div>
                  <div className="flex items-center space-x-1">
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    <span className="text-gray-600">Streak:</span>
                    <span className="font-medium text-gray-900">{user.streak}d</span>
                  </div>
                </div>
              </motion.div>
            ))}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-left py-3 px-4 font-medium text-gray-600 text-sm">Rewards</th>
                  <th className="text-left py-3 px-4 font-medium text-gray-600 text-sm">Goals</th>
                  <th className="text-left py-3 px-4 font-medium text-gray-600 text-sm">Groups</th>
                  <th className="text-left py-3 px-4 font-medium text-gray-600 text-sm">Achievements</th>
                  <th className="text-left py-3 px-4 font-medium text-gray-600 text-sm">Win Rate</th>
                  <th className="text-left py-3 px-4 font-medium text-gray-600 text-sm">Streak</th>
                </tr>
              </thead>
              <tbody>
                {leaderboard.map((user, index) => (
